...
# Initialize OpenAI client with the correct base URL
openai = AsyncOpenAI(
    base_url="https://api.openai.com/v1",  # Explicitly set OpenAI API URL
...
azure_openai = AsyncAzureOpenAI(
    ...
    azure_endpoint=config.AZURE_OPENAI_BASE_URL.rstrip("/"),
...
async def chat_completions(
    ...
    try:
        ...
        # Azure OpenAI Models
        elif data.get("model").startswith("azure/"):
            if azure_openai is None:
                raise HTTPException(
                    status=500, detail="Azure OpenAI API key is not set."
                )
            # Extract the model name from the request
            model_name = data["model"].replace("azure/", "")
            data["model"] = model_name
            try:
                response: AsyncStream[
                    ChatCompletionChunk
                ] = await azure_openai.chat.completions.create(
                    **data,
                )
                return StreamingResponse(
...
async def get_openai_models():
    try:
        if not config.OPENAI_API_KEY or config.OPENAI_API_KEY == "xxx":
            return []
        models = await openai.models.list()
        return [model.id for model in models.data if model.id.startswith("gpt")]
    except Exception:
        logger.warning("Couldn't connect to OpenAI API")
        return []
...
async def get_azure_openai_models():
    try:
        if not config.AZURE_OPENAI_API_KEY:
            logger.warning("Azure OpenAI API key not configured")
            return []
        
        logger.info("Azure OpenAI deployment: %s", config.AZURE_OPENAI_DEPLOYMENT)
        if not config.AZURE_OPENAI_DEPLOYMENT:
            logger.warning("Azure OpenAI deployment not configured")
            return []
            
        # For Azure OpenAI, we'll return the configured deployment
        model = "azure/" + config.AZURE_OPENAI_DEPLOYMENT
        logger.info("Returning Azure OpenAI model: %s", model)
        return [model]
    except Exception as e:
        logger.warning(
            "Couldn't connect to Azure OpenAI at %s: %s", 
