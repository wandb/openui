...
describe('<Settings /> Copilot', () => {
	...
	it('prefers the first Copilot model when OpenAI is unavailable', async () => {
		server.use(
			http.get('/v1/models', () => HttpResponse.json(catalog))
		)
	...
	it('lists Copilot models and marks provider vision metadata read-only', async () => {
		...
		expect(screen.getByRole('switch', { name: 'Supports Vision' })).toBeChecked()
		expect(screen.getByRole('switch', { name: 'Supports Vision' })).toBeDisabled()
		expect(
